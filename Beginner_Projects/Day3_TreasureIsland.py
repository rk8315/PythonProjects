...
while not game_over:
    cross_road_input = input("You're at a cross road that forks in two directions, where do you want to go? (Left or Right): ").lower()
    ...
    lake_input = input("You come to a lake. THere is an island in the middle of the lake. Do you want to wait for a boat or swim across? (Wait or Swim): ").lower()
    if lake_input == "swim":
        print("You are eaten by sharks, Game Over!")
        game_over = True
        break

    house_door_input = input("You arrive at the island unharmed. There is a hourse with 3 doors - red, yellow, and blue. Which color door do you choose to enter? (Red, Yellow, or Blue): ").lower()
